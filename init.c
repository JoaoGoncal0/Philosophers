...
void    init_forks(t_info *info)
{
    ...
    {   
        ...
        pthread_mutex_init(&info->philos[i].state, NULL);
        ptrhead_mutex_init(&info->forks[i].mutex, NULL);
        info->philos[i].info = info;
        info->philos[i].id = i + 1;
        info->philos[i].time_to_die = info->time_to_died;
        info->philos[i].meals_count = 0;
...
int init_program(char **av, t_info *info)
{
    if (check_args(av) == 1)
        return (1);
    info->time_now = get_time();
    info->n_philos = ft_atol(av[1]);
    info->time_to_died = ft_atol(av[2]);
    ...
    info->dead = 0;
    info->meals_eaten = 0;
    info->forks = malloc(sizeof(t_check) * info->n_philos);
...
void    finish_program(t_info *info)
{
    ...
    while (i < info->n_philos)
    {
        pthread_mutex_destroy(&info->philos[i].state);
